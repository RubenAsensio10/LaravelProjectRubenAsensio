    ...
    <div class="p-8 max-w-xl mx-auto">
        ...
        <form action="{{ route('alumnos.update', $alumno) }}" method="POST" class="space-y-4">
            ...
            <div class="form-control">
                ...
                <input type="text" name="nombre" class="input input-bordered" value="{{ old('nombre', $alumno->nombre) }}" required>
            ...
            <div class="form-control">
                ...
                <input type="text" name="apellidos" class="input input-bordered" value="{{ old('apellidos', $alumno->apellidos) }}" required>
                @error('apellidos')<span class="text-error text-sm">{{ $message }}</span>@enderror
            </div>

            <div class="form-control">
                <label class="label"><span class="label-text">Email</span></label>
                <input type="email" name="email" class="input input-bordered" value="{{ old('email', $alumno->email) }}" required>
                @error('email')<span class="text-error text-sm">{{ $message }}</span>@enderror
            </div>

            <div class="form-control">
                <label class="label"><span class="label-text">Fecha de Nacimiento</span></label>
                <input type="date" name="f_nac" class="input input-bordered" value="{{ old('f_nac', $alumno->f_nac->format('Y-m-d')) }}" required>
